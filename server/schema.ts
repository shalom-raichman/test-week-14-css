...
militaryFormSchema.virtual('totalScore').get(function () {
  const combatAvg =
    Object.values(this.combatPreferences).reduce((a, b) => a + b, 0) / 4
  const supportAvg =
    Object.values(this.supportPreferences).reduce((a, b) => a + b, 0) / 4
  const techAvg =
    Object.values(this.techPreferences).reduce((a, b) => a + b, 0) / 4
